...
def validate_common(args):
    if args.until is not None:
        try:
            ...
            raise ValueError(f"until must be formatted as yyyy-mm-dd, got {args.until}")
    if args.radius is not None:
        if args.geo is None:
            raise ValueError("--near be used together with --geo")
        if re.match(RADIUS_REGEX, args.radius) is None:
            raise ValueError("Radius must be a positive integer followed by either 'mi' or 'km' (e.g. '15km')")
    if args.max_tweets <= 0:
        raise ValueError(f"max_tweets must be a positive integer, got {args.max_tweets}")
    validated_args = dict()
...
def validate_snscrape(args, validated_args):
    if args.since is not None:
        try:
            ...
            raise ValueError(f"since must be formatted as yyyy-mm-dd, got {args.since}")
        if dt.strptime(args.since, DATE_FORMAT) >= dt.today():
            raise ValueError(f"since can be at most {dt.strftime(dt.today() - td.timedelta(days=1), '%Y-%m-%d')}, got {args.since}")
...
def validate_tweepy(args, validated_args):
    if args.geo is not None and re.match(f"^{FLOAT_REGEX},{FLOAT_REGEX}$", args.geo) is None:
        raise ValueError(f"geo must have the form 'latitude,longitude' for tweepy, got {args.geo}")
    elif args.geo is not None and args.radius is None:
        raise ValueError(f"If using Tweepy with geolocation, both --geo and --radius must be specified")
